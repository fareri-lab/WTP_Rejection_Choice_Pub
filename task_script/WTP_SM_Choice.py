...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/melanieruiz/Desktop/WTP_SM_Choice/WTP_SM_Choice.py',
...
for thisEntiretaskloop in entiretaskloop:
    ...
    for thisStartWTPloop in startWTPloop:
        ...
        WTPTaskLoop = data.TrialHandler(nReps=1.0, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(wtp_sheet, selection='0:5'),
        ...
        for thisWTPTaskLoop in WTPTaskLoop:
            ...
            blank.setText('')
            # keep track of which components have finished
            WTPTaskComponents = [Left_Experience, Right_Experience, Left_Price, Right_Price, responses, blank]
            ...
            while continueRoutine:
                ...
                if responses.status == STARTED and not waitOnFlip:
                    ...
                    if len(_responses_allKeys):
                        responses.keys = _responses_allKeys[-1].name  # just the last key pressed
                        responses.rt = _responses_allKeys[-1].rt
                
                # *blank* updates
                if blank.status == NOT_STARTED and responses.status== STARTED:
                    # keep track of start time/frame for later
            ...
            continueRoutine= False
            # the Routine "WTPTask" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
            thisExp.nextEntry()
