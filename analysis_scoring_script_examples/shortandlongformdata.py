...
#sheet_type = input("Sheet type? (L/S) ")
...
cols = ['PROLIFIC_ID', 'choice', 'condition', 'salience', 'stress', 'afterstresschange', 'rej-acc', 'ifnegvalue','choicertmean','timebetween', 'age', 'sex','order','playlottery' ]
...
columns3 = ['PROLIFIC_ID', 'condition_recode', 'afterstresschange','salience_mean', 'stress_mean', 'recoded_stress', 'choice','rej-acc', 'ifnegvalue','choicertmean', 'timebetween', 'age', 'sex', 'order', 'choicedifference_rejneu','playlottery']
...
#%%
       ...
       #%%              
            ...
            longform_data = longform_data.append(rej_df[columns3].iloc[[4, 10, 16, 22, 28, 34, 40, 46, 52, 58, 64, 70]]).append(neu_df[columns3].iloc[[4, 10, 16, 22, 28, 34]]).append(acc_df[columns3].iloc[[4, 10, 16, 22, 28,  34, 40, 46, 52, 58, 64, 70]]).reset_index(drop=True)
...
for i in range(0,len(longform_data)):
    if longform_data.loc[i, 'rej-acc'] < 0:
        longform_data['ifnegvalue'][i] = 1 
    else:
        longform_data['ifnegvalue'][i] = 0                        
#%%
...
longform_data=longform_data.sort_values(['PROLIFIC_ID', 'condition_recode']).reset_index(drop=True)
longform_data.to_csv('longformdata_DF.csv', index=False)  
#%%
